<template>
  ...
  <main class="container max-w-screen-xl mx-auto px-4 md:px-6 py-4">
    <div
      class="flex h-[calc(100vh-180px)] border border-gray-200 rounded-lg bg-white dark:bg-gray-800 dark:border-gray-700"
    >
      <!-- 左侧：聊天区域 -->
      <div class="flex-1 flex flex-col">
        ...
        <div class="flex-1 flex flex-col min-h-0">
          <!-- 聊天室标题 -->
          <div
            class="p-4 border-b dark:border-gray-700 flex gap-3 justify-between items-center"
          >
            <div class="flex gap-3">
              <!-- Logo 图标 -->
              <div
                class="w-12 rounded-full bg-sky-100 dark:bg-sky-900 flex items-center justify-center"
              >
                <svg
                  t="1768296965309"
              ...
              <!-- 标题与描述 -->
              <div>
                <h2 class="font-bold text-gray-800 dark:text-gray-200 text-lg">
                  公共聊天室
                </h2>
            ...
            <div
              v-if="
                chatUserStore.chatUserInfo &&
                chatUserStore.chatUserInfo.nickname
              "
              class="flex items-center gap-3"
            >
              <!-- 当前用户头像 -->
              <img
                v-if="chatUserStore.chatUserInfo.avatar"
                :src="chatUserStore.chatUserInfo.avatar"
                :alt="chatUserStore.chatUserInfo.nickname"
                class="w-10 h-10 rounded-full object-cover border-2 border-sky-200 dark:border-sky-800"
              />
              <svg
                v-else
                class="w-10 h-10 text-gray-400 border-2 border-sky-200 dark:border-sky-800 rounded-full"
                aria-hidden="true"
                xmlns="http://www.w3.org/2000/svg"
                fill="currentColor"
                viewBox="0 0 20 20"
          ...
          >
            <!-- 没有更多消息提示 -->
            <div
              v-if="isNoMoreMessages && chatMessages.length > 0"
              class="flex justify-center py-2"
            >
              <span
                class="text-xs text-gray-400 bg-gray-100 dark:bg-gray-800 px-3 py-1 rounded-full"
                >没有更多消息了</span
              >
            </div>
            ...
            <template v-for="message in chatMessages">
              ...
              <div v-if="message.type === 0" class="flex justify-center">
                <span
                  class="text-xs text-gray-500 bg-gray-200 dark:bg-gray-700 dark:text-gray-400 px-4 py-1.5 rounded-full"
                >
                  {{ message.nickname }} {{ message.content }}，当前在线人数:
                  {{ message.onlineCount }}
                </span>
              </div>
              ...
              >
                ...
                <svg
                  v-else
                  class="w-10 h-10 text-gray-400 dark:text-gray-400 flex-shrink-0"
                  aria-hidden="true"
                  xmlns="http://www.w3.org/2000/svg"
                  fill="currentColor"
                  viewBox="0 0 20 20"
                ...
                >
                  <div
                    :class="
                      isMyMessage(message)
                        ? 'flex items-center gap-2 mb-1 flex-row-reverse'
                  ...
                  <div
                    :class="
                      isMyMessage(message)
                        ? 'inline-block bg-sky-500 dark:bg-sky-600 rounded-lg px-4 py-2 shadow-sm'
                        : 'inline-block bg-white dark:bg-gray-800 rounded-lg px-4 py-2 shadow-sm'
                    "
                  >
                    <p
                      :class="
        ...
        <!-- 输入区域 -->
        <div
          class="h-32 border-t border-gray-200 dark:border-gray-700 py-3 px-4 flex flex-col gap-2 bg-white dark:bg-gray-800"
        >
          <!-- 输入框 -->
          <textarea
          ...
          <div class="flex justify-between items-center">
            ...
            >
              ...
              >
                <path
                  d="M512 102.4C286.037333 102.4 102.4 286.378667 102.4 512c0 225.962667 183.637333 409.6 409.6 409.6 225.621333 0 409.6-183.637333 409.6-409.6 0-225.621333-183.978667-409.6-409.6-409.6z m-220.501333 315.733333c0-37.546667 30.378667-68.266667 67.925333-68.266666 37.888 0 68.608 30.72 68.608 68.266666 0 37.888-30.72 68.608-68.608 68.608a68.266667 68.266667 0 0 1-67.925333-68.608z m369.322666 268.629334c-40.96 40.96-95.573333 63.829333-153.6 63.829333s-112.64-22.869333-153.6-63.829333a33.9968 33.9968 0 1 1 48.128-48.128c28.330667 27.989333 65.536 43.690667 105.472 43.690666 39.594667 0 77.141333-15.701333 105.130667-43.690666 13.653333-13.312 35.157333-13.312 48.469333 0 13.312 13.312 13.312 34.816 0 48.128z m2.730667-200.021334a68.266667 68.266667 0 0 1-67.925333-68.608c0-37.546667 30.378667-68.266667 67.925333-68.266666 37.888 0 68.608 30.72 68.608 68.266666 0 37.888-30.72 68.608-68.608 68.608z"
                  p-id="5063"
                ></path>
              </svg>
            </button>
            ...
            <div
              id="emoji-dropdown"
              class="z-10 hidden bg-white rounded-lg shadow-lg dark:bg-gray-700"
            >
            ...
            <button
              @click="sendMessage"
              :disabled="
                !ws || ws.readyState !== WS_OPEN || !messageInput.trim()
              "
      ...
      <!-- 右侧：成员列表 -->
      <div
        class="w-64 border-l border-gray-200 dark:border-gray-700 flex flex-col"
      >
        <!-- 成员列表标题 -->
        <div class="p-4 border-b dark:border-gray-700 flex gap-3">
          ...
          >
            ...
            >
              <path
                d="M500.6 513.2c44.7-35 73.4-89.5 73.4-150.7 0-105.7-85.6-191.3-191.3-191.3s-191.3 85.7-191.3 191.3c0 61.7 29.3 116.6 74.7 151.6C147.8 563.6 64.3 684 64.3 818.7v0.1h0.1c0 17.7 14.5 32 32.2 32 17.8 0 32.2-14.4 32.2-32.1 0-1.1-0.1-2.2-0.2-3.3 2.3-139 115.9-260 255.8-260 140.3 0 254.3 121.7 255.9 261.3 0 0.6-0.1 1.3-0.1 1.9 0 17.7 14.4 32.1 32.2 32.1 17.8 0.1 32.2-14.2 32.3-32 0-135.5-84.7-256.7-204.1-305.5zM382.7 233.3c70.8 0 128.3 57.4 128.3 128.2 0 70.9-57.4 128.4-128.3 128.4s-128.3-57.5-128.3-128.3c0-70.9 57.5-128.3 128.3-128.3z"
                p-id="5628"
              ></path>
              <path
                d="M896.3 799.6a32.2 32.1 0 1 0 64.4 0 32.2 32.1 0 1 0-64.4 0Z"
                p-id="5629"
              ></path>
              <path
                d="M931 675.6c0-6.5-2-12.8-5.6-17.9-35.6-65-95.7-114.5-168.8-144.5 44.7-35 73.4-89.5 73.4-150.7 0-82.5-52.3-152.9-125.5-179.7-0.4-0.1-0.8-0.3-1.2-0.4-2.8-0.8-5.8-1.3-8.9-1.3-17.2 0-31.2 13.9-31.2 31 0 13.9 9.2 25.7 21.8 29.6 48 18.6 82 65.2 82 119.7 0 52.8-31.8 98.1-77.3 117.8-15.6 6.7-26.5 22.2-26.5 40.2 0 20.8 14.6 38.3 34.1 42.7 76.5 18.5 140 64.4 173.5 129.3 5.5 9.7 16 16.3 28 16.3 17.8 0 32.2-14.4 32.2-32.1z"
                p-id="5630"
              ></path>
            </svg>
          </div>
        ...
        <div class="flex-1 overflow-y-auto p-4 space-y-1">
          ...
          <template v-if="onlineUsers && onlineUsers.length > 0">
            ...
            >
              ...
              <div class="relative">
                ...
                <svg
                  v-else
                  class="w-10 h-10 text-gray-400 dark:text-gray-400 border-2 border-green-400 rounded-full"
                  aria-hidden="true"
                  xmlns="http://www.w3.org/2000/svg"
                  fill="currentColor"
                  viewBox="0 0 20 20"
          ...
          >
            ...
            >
              <path
                d="M0.512 0h1024v1024H0.512z"
                fill="currentColor"
                fill-opacity=".01"
                p-id="5156"
              ></path>
              <path
                d="M380.974545 83.316364a246.318545 246.318545 0 0 1 156.439273 436.642909 385.303273 385.303273 0 0 1 224.069818 349.556363 45.149091 45.149091 0 1 1-90.251636 0c0-160.256-129.954909-293.515636-290.257455-293.515636-160.256 0-290.210909 133.259636-290.210909 293.562182a45.149091 45.149091 0 1 1-90.251636 0 385.536 385.536 0 0 1 224.069818-349.696 246.318545 246.318545 0 0 1 156.392727-436.549818z m263.07491-19.316364a246.318545 246.318545 0 0 1 158.440727 434.920727c131.025455 56.785455 222.021818 183.063273 222.021818 336.989091a45.149091 45.149091 0 0 1-90.251636 0c0-132.701091-100.538182-248.040727-233.890909-273.873454a381.393455 381.393455 0 0 0-79.36-97.373091 156.113455 156.113455 0 1 0 2.373818-309.061818 45.428364 45.428364 0 0 1-22.574546-87.645091c0.139636-0.186182 0.325818-0.232727 0.512-0.279273 13.870545-2.420364 28.16-3.677091 42.728728-3.677091zM380.974545 173.568a156.113455 156.113455 0 1 0 0 312.226909 156.113455 156.113455 0 0 0 0-312.226909z"
                fill="currentColor"
                p-id="5157"
              ></path>
            </svg>
      ...
      <div class="relative bg-white rounded-lg shadow-sm dark:bg-gray-700">
          ...
          <div class="mt-4">
            ...
            <div v-show="activeTab === 'qq'" class="p-4">
              <div class="flex gap-2">
                ...
                <input
                  v-model="qqInput"
                  class="bg-gray-50 border outline-none border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-sky-500 focus:border-sky-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-sky-500 dark:focus:border-sky-500"
                  placeholder="请输入你的 QQ 号"
                />
                <!-- 获取按钮 -->
                <button
                  type="button"
                  @click="handleGetQQUserInfo"
                  :disabled="isLoading"
                  class="text-white bg-sky-700 hover:bg-sky-800 focus:ring-4 focus:outline-none focus:ring-sky-300 font-medium rounded-lg text-sm px-5 py-2.5 whitespace-nowrap dark:bg-sky-600 dark:hover:bg-sky-700 dark:focus:ring-sky-800 disabled:opacity-50 disabled:cursor-not-allowed"
                >
              ...
              <div
                ...
                class="mt-4 p-4 bg-gray-50 dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-600"
              >
        ...
        >
          <button
            type="button"
            @click="closeNicknameModal"
            class="flex-1 py-2.5 text-sm font-medium text-gray-900 focus:outline-none bg-white rounded-lg border border-gray-200 hover:bg-gray-100 hover:text-sky-700 focus:z-10 focus:ring-4 focus:ring-gray-100 dark:focus:ring-gray-700 dark:bg-gray-800 dark:text-gray-400 dark:border-gray-600 dark:hover:text-white dark:hover:bg-gray-700"
          >
            算了吧
          </button>
          <!-- 加入聊天室按钮 -->
          <button
            type="button"
            @click="joinChatRoom"
            class="flex-1 text-white bg-sky-700 hover:bg-sky-800 focus:ring-4 focus:outline-none focus:ring-sky-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center dark:bg-sky-600 dark:hover:bg-sky-700 dark:focus:ring-sky-800"
          >
            加入聊天室
          </button>
        </div>
...
const handleGetQQUserInfo = () => {
  ...
  getUserInfoByQQ(qqInput.value.trim())
    .then((res) => {
      // 若响应失败
      if (!res.success) {
        // 获取服务端返回的错误消息
...
const loadMoreChatHistory = (lastId) => {
  ...
  // 调用接口加载更多数据
  getChatHistory(lastId, mySessionId.value)
    .then((res) => {
      if (res.success) {
