<template>
    <div class="main min-h-screen flex flex-col">
        ...
        <main class="grow container max-w-screen-3xl mx-auto px-4 sm:px-6 md:px-8 py-4">
            <!-- 左边栏 -->
            <div class="transition-all duration-300 hidden lg:block fixed top-[5.5rem] bottom-0
                 pb-10 overflow-y-auto"
                :class="[isExpand ? 'left-[max(0px,calc(50%-45rem))] w-[20rem] pl-8 pr-6' : 'left-0 w-0 pl-0 pr-0']"
                >

                <div class="flex">
                    <!-- 知识库目录 -->
                    <div class="grow transition-all duration-300 overflow-hidden" :class="[isExpand ? 'block w-full' : 'hidden w-0']">
            ...
            <!-- 收缩展开按钮（固定在左侧） -->
            <div class="hidden lg:block fixed top-[5.5rem] z-50" 
                 :class="[isExpand ? 'left-[max(0px,calc(50%-45rem))] translate-x-[19rem]' : 'left-0']"
                 @click="shrinkAndExpand">
                <div id="left-toc-sidebar" class="left-toc-sidebar w-8 h-10 flex items-center justify-center cursor-pointer hover:bg-gray-100 dark:hover:bg-gray-800 rounded-r">
            ...
            <!-- 中间栏 -->
            <div class="transition-all duration-300" :class="[isExpand ? 'lg:pl-[22.5rem]' : 'lg:pl-12']">
