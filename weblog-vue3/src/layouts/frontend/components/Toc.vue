    ...
    <div class="border-l-2 border-gray-200">
      <ul>
        ...
        <li
          ...
          :key="index"
          class="pl-5 transition-all duration-300 cursor-pointer hover:text-sky-500 overflow-hidden"
          :class="[
            h2.index == activeHeadingIndex
              ? 'active text-sky-600 border-l-2 border-sky-600 font-bold'
              : 'text-gray-500 font-normal',
          ]"
        >
          <span
            @click="scrollToView(h2.offsetTop)"
            class="truncate hover:whitespace-normal hover:break-all transition-all duration-300"
            >{{ h2.text }}</span
          >
          <!-- 三级标题 -->
